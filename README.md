...
Если в логах видно `535 5.7.8 ... This user does not have access rights to this service`, чаще всего:

1. **`SMTP_FROM` и `SMTP_USER` — разные ящики.** Вход выполняется под одним логином, а в команде `MAIL FROM` уходит другой адрес — Яндекс это запрещает. Задайте один и тот же адрес в обеих переменных или добавьте второй адрес как **алиас** в настройках того ящика, под которым создан пароль приложения.
2. **Пароль приложения** создаётся в [Яндекс ID](https://id.yandex.ru/security) для **конкретного** аккаунта; в настройках Почты должен быть включён доступ по **SMTP**.

При несовпадении `SMTP_FROM` и `SMTP_USER` для хоста `smtp.yandex.ru` приложение теперь **не стартует** и выводит понятную ошибку конфигурации.
