...
### Яндекс.Pочта и ошибка 535 «access rights»

В логах приложения префикс **`smtp auth:`** означает, что отказ пришёл на этапе **SMTP AUTH** (ещё до отправки письма). Тогда сообщение `535 5.7.8 ... This user does not have access rights to this service` почти всегда связано с **настройками ящика в Яндексе**, а не с кодом.

1. **`SMTP_FROM` и `SMTP_USER` — разные ящики** (если совпадают, этот пункт можно пропустить). Задайте один адрес в обеих переменных или добавьте алиас в настройках ящика, для которого создан пароль приложения.
2. **Доступ для почтовых программ.** В [Почте](https://mail.yandex.ru/) → **Все настройки** → **Почтовые программы** включите:
   - получение почты **с сервера IMAP** (`imap.yandex.com` / для РФ также актуален вариант из подсказок интерфейса);
   - **Пароли приложений и OAuth-токены** (если в `SMTP_PASS` указан не основной пароль аккаунта, а пароль приложения).
3. **Пароль приложения** создаётся в [Яндекс ID → Безопасность](https://id.yandex.ru/security) для **того же** аккаунта, что и `SMTP_USER`; в качестве логина SMTP указывается **полный** адрес почты.
4. **Пользовательское соглашение:** хотя бы один раз войдите в веб-интерфейс Почты этим аккаунтом (соглашение принимается при входе).
5. **Хост SMTP:** в справке Яндекса указан `smtp.yandex.com`, порт **465** с SSL; `smtp.yandex.ru` обычно тоже работает, при сбоях имеет смысл попробовать `SMTP_HOST=smtp.yandex.com`.
6. **Docker:** после правки `.env` пересоздайте контейнер приложения, иначе внутри может остаться старый набор переменных:  
   `docker compose up -d --build --force-recreate app`  
   В режиме **`APP_ENV=development`** при старте в лог пишется строка `SMTP (без секретов): ...` — сверьте, что там те же `user`/`from`, что в вашем `.env`.

При несовпадении адреса из `SMTP_FROM` с `SMTP_USER` для хоста с подстрокой `yandex` в имени приложение **не стартует** и сообщает об этом при загрузке конфигурации.

**Адрес получателя** (новый или «существующий» в базе приложения) на ответ SMTP **не влияет**: ошибка `smtp auth: 535 ...` возникает **до** отправки письма, на шаге входа к серверу Яндекса.

Дополнительно можно задать **`SMTP_AUTH=login`** (вместо `plain` по умолчанию) — используется механизм **AUTH LOGIN**; иногда это помогает при сбоях с **AUTH PLAIN**. Пробелы в начале/конце значений `SMTP_HOST`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM` при загрузке конфигурации обрезаются.
